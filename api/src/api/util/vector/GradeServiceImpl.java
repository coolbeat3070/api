...
public class GradeServiceImpl implements GradeService {
	...
	private List<GradeVO> lists = new Vector<GradeVO>();
	...
	public void print() {
		...
		Iterator<GradeVO> it = lists.iterator();
		while (it.hasNext()) {
			GradeVO vo = it.next(); // 다운 캐스팅이 필요 없다.

			// toString() 오버라이딩 지정 전
			/*
			 * System.out.printf("%s %s %d %d %d%n" //,vo.hak, vo.name //->오류.
			 * 접근제어자가 private이기 때문에. //-> getter를 이용해서 접근할수 있다. , vo.getHak(),
			...
			// toString() 오버라이딩 지정 후
			System.out.println(vo.toString()); // toString이 설정 안되었을시 클래스이름만
												// 출력된다.
		}
	}
	...
	@Override
	public void searchGradeByName(String name) {
		// 이름을 기준으로 검색.
	...
	public void descSortTot() {
		...
		Comparator<GradeVO> comp = new Comparator<GradeVO>() {
